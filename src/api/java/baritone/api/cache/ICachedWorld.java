...
public interface ICachedWorld {
	 ...
	 * of the chunk, then packing the data into the 2-bit format, and storing
	 * that in this cached world.
	 *
	 ...
	 * information that is returned by this method may not be up to date,
	 * because older cached chunks can contain data that is much more likely to
	 * have changed.
	 *
	 * @param block               The special block to search for
	 * @param maximum             The maximum number of position results to
	 * receive
	 ...
	 */
	ArrayList<BlockPos> getLocationsOf(String block, int maximum, int centerX,
		int centerZ, int maxRegionDistanceSq);

	/**
	 * Reloads all of the cached regions in this world from disk. Anything that
	 * is not saved will be lost. This operation does not execute in a new
	 * thread by default.
	 */
	void reloadAllFromDisk();

	/**
	 * Saves all of the cached regions in this world to disk. This operation
	 * does not execute in a new thread by default.
	 */
	void save();
}
