 ...
 * target class. It can be assumed that a {@link IArgParser} exists for {@link
 * Integer}, {@link Long},
...
public interface IArgParserManager {
	 ...
	 * @return A parser that can parse arguments into this class, if found.
	 */
	<T, S> IArgParser.Stated<T, S> getParserStated(
		Class<T> type, Class<S> stateKlass);

	/**
	 * Attempt to parse the specified argument with a stateless {@link
	 ...
	 */
	<T> T parseStateless(Class<T> type, ICommandArgument arg)
		throws CommandInvalidTypeException;

	/**
	 * Attempt to parse the specified argument with a stated {@link IArgParser}
	 ...
	 */
	<T, S> T parseStated(Class<T> type, Class<S> stateKlass,
