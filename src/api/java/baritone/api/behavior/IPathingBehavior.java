 ...
 * cost of the currently executing movement in the total
 * @return The estimated remaining ticks in the current segment.
 */
default Optional
	<Double> ticksRemainingInSegment(boolean includeCurrentMovement) {
		IPathExecutor current = getCurrent();
		if(current == null) {
			return Optional.empty();
		}
		int start = includeCurrentMovement ? current.getPosition()
	...
	/**
	 * @return Whether or not a path is currently being executed. This will be
	 * false if there's currently a pause.
	 * @see #hasPath()
	 */
	boolean isPathing();
 ...
 * being executed, for example when there is a pause in effect.
 * @see #isPathing()
 */
default boolean
	hasPath() {
	 ...
	 * processes that could be controlling path. <p> Basically, "MAKE IT STOP".
	 *
	 * @return Whether or not the pathing behavior was canceled. All processes
	 * are guaranteed to be canceled, but the PathingBehavior might be in the
	 * middle of an uncancelable action like a parkour jump
	 */
	 ...
	 * <p>
	 * If cancelEverything was like "kill" this is "sudo kill -9". Or shutting
	 * off your computer.
	 */
	void forceCancel();
