...
public interface IBuilderProcess extends IBaritoneProcess {
	...
	/**
	 * Requests a build for the specified schematic, labeled as specified, with
...
default boolean
	build(String schematicFile, BlockPos origin) {
		File file
			= new File(new File(Minecraft.getInstance().gameDir, "schematics"),
				schematicFile);
	 ...
	 * builder process. You can use this in schematics, for example, to pick a
	 * state that the builder process will be happy with, because any variation
	 * will cause it to give up. This is updated every tick, but only while the
