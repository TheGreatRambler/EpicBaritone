...
public interface Helper {
	...
	static ITextComponent getPrefix() {
		...
		final boolean xd   = now.get(Calendar.MONTH) == Calendar.APRIL
						   && now.get(Calendar.DAY_OF_MONTH) <= 3;
		TextComponent baritone = new StringTextComponent(
			xd ? "Baritoe"
			   : BaritoneAPI.getSettings().shortBaritonePrefix.value
					 ? "B"
					 : "Baritone");
		baritone.setStyle(
			baritone.getStyle().setFormatting(TextFormatting.LIGHT_PURPLE));

		// Outer brackets
		TextComponent prefix = new StringTextComponent("");
		prefix.setStyle(
			baritone.getStyle().setFormatting(TextFormatting.DARK_PURPLE));
		prefix.appendString("[");
 ...
 */
default void
	logDirect(ITextComponent... components) {
 ...
 * critically important messages, or as a direct response to a chat command)
 *
 * @param message The message to display in chat
 * @param color   The color to print that message in
 */
default void
	logDirect(String message, TextFormatting color) {
		Stream.of(message.split("\n")).forEach(line -> {
			TextComponent component
				= new StringTextComponent(line.replace("\t", "    "));
