 ...
 * performant offsets) <p> Is it really faster to subclass BlockPos and
 * calculate a hash in the constructor like this, taking everything into
 * account? Yes. 20% faster actually. It's called BETTER BlockPos for a reason.
 * Source: <a
...
public final class BetterBlockPos extends BlockPos {
	...
	/**
	 * Like constructor but returns null if pos is null, good if you just need
	 * to possibly censor coordinates
	 *
	...
	public static long longHash(int x, int y, int z) {
		...
		/*
		 *   This is the hashcode implementation of Vec3i (the superclass of the
		 * class which I shall not name)
		 *
		 *   public int hashCode() {
		 *       return (this.getY() + this.getZ() * 31) * 31 + this.getX();
		 *   }
		 *
		 *   That is terrible and has tons of collisions and makes the HashMap
	...
	public BetterBlockPos up() {
		...
		// facing.getXOffset() * n, this.getY() + facing.getYOffset() * n,
		// this.getZ() + facing.getZOffset() * n);

		// how many function calls is that? up(), up(int), offset(Direction,
		// int), new BlockPos, getX, getXOffset, getY, getYOffset, getZ,
		// getZOffset that's ten. this is one function call.
	...
	public BetterBlockPos offset(Direction dir, int dist) {
		...
		Vector3i vec = dir.getDirectionVec();
		return new BetterBlockPos(x + vec.getX() * dist, y + vec.getY() * dist,
			z + vec.getZ() * dist);
	}
	...
	public String toString() {
		return String.format("BetterBlockPos{x=%s,y=%s,z=%s}",
