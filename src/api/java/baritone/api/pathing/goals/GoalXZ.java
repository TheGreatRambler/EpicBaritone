...
public class GoalXZ implements Goal {
	...
	public double heuristic(
		int x, int y, int z) { // mostly copied from GoalBlock
		int xDiff = x - this.x;
		int zDiff = z - this.z;
	...
	public String toString() {
		return String.format("GoalXZ{x=%s,z=%s}", SettingsUtil.maybeCensor(x),
			SettingsUtil.maybeCensor(z));
	}

	public static double calculate(double xDiff, double zDiff) {
		// This is a combination of pythagorean and manhattan distance
		// It takes into account the fact that pathing can either walk
		// diagonally or forwards
		...
		return (diagonal + straight)
			* BaritoneAPI.getSettings().costHeuristic.value; // big TODO tune
	}

	public static GoalXZ fromDirection(
		Vector3d origin, float yaw, double distance) {
