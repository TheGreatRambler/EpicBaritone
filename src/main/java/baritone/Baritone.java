...
public class Baritone implements IBaritone {
	...
	static {
		threadPool = new ThreadPoolExecutor(4, Integer.MAX_VALUE, 60L,
	...
	Baritone() {
		...
		// Define this before behaviors try and get it, or else it will be null
		// and the builds will fail!
		this.playerContext = PrimaryPlayerContext.INSTANCE;

		{
			// the Behavior constructor calls baritone.registerBehavior(this) so
			// this populates the behaviors arraylist
		...
		if(BaritoneAutoTest.ENABLE_AUTO_TEST) {
			this.gameEventHandler.registerEventListener(
				BaritoneAutoTest.INSTANCE);
		}
	}
	...
	public void openClick() {
		new Thread(() -> {
			try {
				Thread.sleep(100);
				Helper.mc.execute(
					() -> Helper.mc.displayGuiScreen(new GuiClick()));
