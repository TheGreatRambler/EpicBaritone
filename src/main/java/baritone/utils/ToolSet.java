...
public class ToolSet {
	...
	public double getStrVsBlock(BlockState state) {
		return breakStrengthCache.computeIfAbsent(
			state.getBlock(), backendCalculation);
	}

	/**
	 * Evaluate the material cost of a possible tool. Will return 1 for tools,
	 * -1 for other
	 *
	...
	private double getBestDestructionTime(Block b) {
		ItemStack stack
			= player.inventory.getStackInSlot(getBestSlot(b, false));
		return calculateSpeedVsBlock(stack, b.getDefaultState())
			* avoidanceMultiplier(b);
	}

	private double avoidanceMultiplier(Block b) {
		return Baritone.settings().blocksToAvoidBreaking.value.contains(b) ? 0.1
																		   : 1;
	}

	/**
	 * Calculates how long would it take to mine the specified block given the
	 * best tool in this toolset is used. A negative value is returned if the
	 * specified block is unbreakable.
	 *
	 * @param item  the item to mine it with
	 * @param state the blockstate to be mined
	 * @return how long it would take in ticks
	 */
	public static double calculateSpeedVsBlock(
		...
		if(speed > 1) {
			int effLevel = EnchantmentHelper.getEnchantmentLevel(
				Enchantments.EFFICIENCY, item);
		...
		if(!state.getRequiresTool()
			|| (!item.isEmpty() && item.canHarvestBlock(state))) {
			return speed / 30;
		} else {
			return speed / 100;
		}
	...
	private double potionAmplifier() {
		...
		if(player.isPotionActive(Effects.HASTE)) {
			speed
				*= 1
				   + (player.getActivePotionEffect(Effects.HASTE).getAmplifier()
						 + 1)
						 * 0.2;
		}
		if(player.isPotionActive(Effects.MINING_FATIGUE)) {
			switch(player.getActivePotionEffect(Effects.MINING_FATIGUE)
					   .getAmplifier()) {
			case 0:
				speed *= 0.3;
				break;
			case 1:
				...
				break;
			case 2:
				speed *= 0.0027; // you might think that 0.09*0.3 = 0.027 so
								 // that should be next, that would make too
								 // much sense. it's 0.0027.
				break;
			default:
				speed *= 0.00081;
				break;
