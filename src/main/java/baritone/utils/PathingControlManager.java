...
public class PathingControlManager implements IPathingControlManager {
	...
	public PathingControlManager(Baritone baritone) {
		...
		baritone.getGameEventHandler().registerEventListener(
			new AbstractGameEventListener() { // needs to be after all behavior
											  // ticks
				@Override
	...
	public void
	cancelEverything() { // called by PathingBehavior on TickEvent Type OUT
		...
		for(IBaritoneProcess proc : processes) {
			proc.onLostControl();
			if(proc.isActive()
				&& !proc.isTemporary()) { // it's okay only for a temporary
										  // thing (like combat pause) to
										  // maintain control even if you say to
										  // cancel
	...
	public void preTick() {
		...
		if(!Objects.equals(inControlThisTick, inControlLastTick)
			...
			&& inControlLastTick != null && !inControlLastTick.isTemporary()) {
			// if control has changed from a real process to another real
			// process, and the new process wants to do something
			p.cancelSegmentIfSafe();
		...
		case SET_GOAL_AND_PATH:
			...
			if(command.goal != null) {
				baritone.getPathingBehavior().secretInternalSetGoalAndPath(
					command);
			}
			break;
		default:
	...
	private void postTick() {
		...
		// therefore, we only cancel and recalculate after the tick for the
		// current path has executed "it would suck" means it would actually
		// execute a path every other tick
		if(command == null) {
			return;
		}
		PathingBehavior p = baritone.getPathingBehavior();
		switch(command.commandType) {
		case FORCE_REVALIDATE_GOAL_AND_PATH:
			if(command.goal == null || forceRevalidate(command.goal)
	...
	public boolean forceRevalidate(Goal newGoal) {
		...
		if(current != null) {
			...
			return !newGoal.toString().equals(
				current.getPath().getGoal().toString());
		}
		return false;
	}
	...
	public PathingCommand executeProcesses() {
		...
		active.sort(
			Comparator.comparingDouble(IBaritoneProcess::priority).reversed());
