...
public class BlockStateInterface {
	...
	public BlockStateInterface(
		World world, WorldData worldData, boolean copyLoadedChunks) {
		this.world     = world;
		this.worldData = worldData;
		if(copyLoadedChunks) {
			this.provider = ((IClientChunkProvider)world.getChunkProvider())
	...
	public static Block getBlock(IPlayerContext ctx,
		BlockPos pos) { // won't be called from the pathing thread because the
						// pathing thread doesn't make a single blockpos pog
		return get(ctx, pos).getBlock();
	}

	public static BlockState get(IPlayerContext ctx, BlockPos pos) {
		return new BlockStateInterface(ctx).get0(
			pos.getX(), pos.getY(), pos.getZ()); // immense iq
		// can't just do world().get because that doesn't work for out of bounds
		// and toBreak and stuff fails when the movement is instantiated out of
		// load range but it's not able to BlockStateInterface.get what it's
		// going to walk on
	}
	...
	public BlockState get0(int x, int y, int z) { // Mickey resigned
		...
		if(useTheRealWorld) {
			...
			// see issue #113
			if(cached != null && cached.getPos().x == x >> 4
				&& cached.getPos().z == z >> 4) {
				return getFromChunk(cached, x, y, z);
			}
			Chunk chunk
				= provider.getChunk(x >> 4, z >> 4, ChunkStatus.FULL, false);
			if(chunk != null && !chunk.isEmpty()) {
				prev = chunk;
				return getFromChunk(chunk, x, y, z);
			}
		}
		// same idea here, skip the Long2ObjectOpenHashMap.get if at all
		// possible except here, it's 512x512 tiles instead of 16x16, so even
		// better repetition
		CachedRegion cached = prevCached;
		if(cached == null || cached.getX() != x >> 9
			|| cached.getZ() != z >> 9) {
			if(worldData == null) {
				return AIR;
			}
	...
	public boolean isLoaded(int x, int z) {
		...
		if(prevChunk != null && prevChunk.getPos().x == x >> 4
			&& prevChunk.getPos().z == z >> 4) {
			return true;
		}
		...
		if(prevRegion != null && prevRegion.getX() == x >> 9
			&& prevRegion.getZ() == z >> 9) {
	...
	// get the block at x,y,z from this chunk WITHOUT creating a single blockpos
	// object
