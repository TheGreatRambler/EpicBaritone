...
public class GuiClick extends Screen implements Helper {
	...
	public void render(
		...
		my *= mc.getMainWindow().getFramebufferHeight()
			  / (double)mc.getMainWindow().getHeight();
		mx *= mc.getMainWindow().getFramebufferWidth()
			  / (double)mc.getMainWindow().getWidth();
		Vector3d near = toWorld(mx, my, 0);
		Vector3d far  = toWorld(
            mx, my, 1); // "Use 0.945 that's what stack overflow says" - leijurv

		if(near != null && far != null) {
			///
			Vector3d viewerPos = new Vector3d(
				PathRenderer.posX(), PathRenderer.posY(), PathRenderer.posZ());
			ClientPlayerEntity player = BaritoneAPI.getProvider()
											.getPrimaryBaritone()
											.getPlayerContext()
											.player();
			RayTraceResult result = player.world.rayTraceBlocks(
				new RayTraceContext(near.add(viewerPos), far.add(viewerPos),
		...
		if(mouseButton == 0) {
			if(clickStart != null && !clickStart.equals(currentMouseOver)) {
				...
				BaritoneAPI.getProvider()
					...
					.setGoalAndPath(new GoalTwoBlocks(currentMouseOver));
			}
	...
	public void onRender(
		MatrixStack modelViewStack, Matrix4f projectionMatrix) {
		...
		if(currentMouseOver != null) {
			...
			PathRenderer.drawManySelectionBoxes(modelViewStack, e,
				Collections.singletonList(currentMouseOver), Color.CYAN);
			if(clickStart != null && !clickStart.equals(currentMouseOver)) {
				RenderSystem.enableBlend();
				RenderSystem.blendFuncSeparate(
					GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ZERO);
				RenderSystem.color4f(Color.RED.getColorComponents(null)[0],
					Color.RED.getColorComponents(null)[1],
					Color.RED.getColorComponents(null)[2], 0.4F);
				...
				IRenderer.drawAABB(modelViewStack,
					new AxisAlignedBB(Math.min(a.x, b.x), Math.min(a.y, b.y),
						Math.min(a.z, b.z), Math.max(a.x, b.x) + 1,
						Math.max(a.y, b.y) + 1, Math.max(a.z, b.z) + 1));
