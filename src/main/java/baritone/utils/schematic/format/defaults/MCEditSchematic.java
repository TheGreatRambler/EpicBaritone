	...
	public MCEditSchematic(CompoundNBT schematic) {
		...
		if(schematic.contains("AddBlocks")) {
			...
			for(int i = 0; i < addBlocks.length; i++) {
				additional[i * 2 + 0]
					= (byte)((addBlocks[i] >> 4) & 0xF); // lower nibble
				additional[i * 2 + 1]
					= (byte)((addBlocks[i] >> 0) & 0xF); // upper nibble
			}
		}
		...
		for(int y = 0; y < this.y; y++) {
			for(int z = 0; z < this.z; z++) {
				for(int x = 0; x < this.x; x++) {
					...
					if(additional != null) {
						// additional is 0 through 15 inclusive since it's & 0xF
						// above
						blockID |= additional[blockInd] << 8;
					}
					Block block = Registry.BLOCK.getOrDefault(
						ResourceLocation.tryCreate(
							ItemIntIDToString.getItem(blockID)));
