...
public final class SpongeSchematic extends StaticSchematic {
	...
	public SpongeSchematic(CompoundNBT nbt) {
		...
		for(String tag : paletteTag.keySet()) {
			...
			SerializedBlockState serializedState
				= SerializedBlockState.getFromString(tag);
			if(serializedState == null) {
				throw new IllegalArgumentException(
					"Unable to parse palette tag");
			}

			BlockState state = serializedState.deserialize();
			if(state == null) {
				throw new IllegalArgumentException(
					"Unable to deserialize palette tag");
			}

			palette.put(index, state);
		}

		// BlockData is stored as an NBT byte[], however, the actual data that
		// is represented is a varint[]
	...
	private static final class SerializedBlockState {
		...
		private BlockState deserialize() {
			if(this.blockState == null) {
				Block block
					= Registry.BLOCK.getOrDefault(this.resourceLocation);
				...
				this.properties.keySet()
					...
					.forEachOrdered(key -> {
						Property<?> property
							= block.getStateContainer().getProperty(key);
						if(property != null) {
							this.blockState = setPropertyValue(this.blockState,
		...
		private static SerializedBlockState getFromString(String s) {
			...
			try {
				...
				return new SerializedBlockState(
					resourceLocation, propertiesMap);
			} catch(Exception e) {
				e.printStackTrace();
				return null;
			}
