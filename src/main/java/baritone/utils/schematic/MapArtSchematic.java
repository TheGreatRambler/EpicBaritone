...
public class MapArtSchematic extends MaskSchematic {
	...
	private static int[][] generateHeightMap(IStaticSchematic schematic) {
		...
		for(int x = 0; x < schematic.widthX(); x++) {
			for(int z = 0; z < schematic.lengthZ(); z++) {
				...
				OptionalInt lowestBlockY = lastIndexMatching(
					column, state -> !(state.getBlock() instanceof AirBlock));
				if(lowestBlockY.isPresent()) {
					heightMap[x][z] = lowestBlockY.getAsInt();
				} else {
					System.out.println(
						"Column " + x + "," + z
						+ " has no blocks, but it's apparently map art? wtf");
	...
	private static <T> OptionalInt lastIndexMatching(
		T[] arr, Predicate<? super T> predicate) {
