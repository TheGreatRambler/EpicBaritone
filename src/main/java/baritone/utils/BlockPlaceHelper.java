...
public class BlockPlaceHelper implements Helper {
	...
	public void tick(boolean rightClickRequested) {
		...
		if(!rightClickRequested || ctx.player().isRowingBoat()
			|| mouseOver == null
		...
		for(Hand hand : Hand.values()) {
			if(ctx.playerController().processRightClickBlock(ctx.player(),
				   ctx.world(), hand, (BlockRayTraceResult)mouseOver)
				== ActionResultType.SUCCESS) {
				ctx.player().swingArm(hand);
				return;
			}
			if(!ctx.player().getHeldItem(hand).isEmpty()
				&& ctx.playerController().processRightClick(
					   ctx.player(), ctx.world(), hand)
					   == ActionResultType.SUCCESS) {
				return;
			}
		}
