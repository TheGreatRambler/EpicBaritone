...
public final class InputOverrideHandler
	extends Behavior implements IInputOverrideHandler {
	...
	public InputOverrideHandler(Baritone baritone) {
		super(baritone);
		this.blockBreakHelper
			= new BlockBreakHelper(baritone.getPlayerContext());
		this.blockPlaceHelper
			= new BlockPlaceHelper(baritone.getPlayerContext());
	}
	...
	public final boolean isInputForcedDown(Input input) {
		return input == null
			? false
			: this.inputForceStateMap.getOrDefault(input, false);
	}
	...
	public final void onTick(TickEvent event) {
		...
		if(inControl()) {
			if(ctx.player().movementInput.getClass()
				!= PlayerMovementInput.class) {
				ctx.player().movementInput = new PlayerMovementInput(this);
			}
		} else {
			if(ctx.player().movementInput.getClass()
				== PlayerMovementInput
					   .class) { // allow other movement inputs that aren't this
								 // one, e.g. for a freecam
				ctx.player().movementInput = new MovementInputFromOptions(
					Minecraft.getInstance().gameSettings);
			}
		}
		// only set it if it was previously incorrect
		// gotta do it this way, or else it constantly thinks you're beginning a
		// double tap W sprint lol
	}

	private boolean inControl() {
		for(Input input : new Input[] { Input.MOVE_FORWARD, Input.MOVE_BACK,
				Input.MOVE_LEFT, Input.MOVE_RIGHT, Input.SNEAK }) {
			if(isInputForcedDown(input)) {
				return true;
			}
		}
		// if we are not primary (a bot) we should set the movementinput even
		// when idle (not pathing)
