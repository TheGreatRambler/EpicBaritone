...
public final class Favoring {
	...
	public Favoring(
		IPlayerContext ctx, IPath previous, CalculationContext context) {
	...
	public Favoring(IPath previous,
		CalculationContext
			context) { // create one just from previous path, no mob avoidances
		...
		if(coeff != 1D && previous != null) {
			previous.positions().forEach(
				pos -> favorings.put(BetterBlockPos.longHash(pos), coeff));
		}
	}
