...
public class Avoidance {
	...
	public Avoidance(
		int centerX, int centerY, int centerZ, double coefficient, int radius) {
	...
	public static List<Avoidance> create(IPlayerContext ctx) {
		...
		double mobSpawnerCoeff
			= Baritone.settings().mobSpawnerAvoidanceCoefficient.value;
		double mobCoeff = Baritone.settings().mobAvoidanceCoefficient.value;
		if(mobSpawnerCoeff != 1.0D) {
			ctx.worldData()
				.getCachedWorld()
				.getLocationsOf(
					"mob_spawner", 1, ctx.playerFeet().x, ctx.playerFeet().z, 2)
				.forEach(mobspawner
					-> res.add(new Avoidance(mobspawner, mobSpawnerCoeff,
			...
			ctx.entitiesStream()
				...
				.filter(entity
					-> (!(entity instanceof SpiderEntity))
						   || ctx.player().getBrightness() < 0.5)
				.filter(entity
					-> !(entity instanceof ZombifiedPiglinEntity)
						   || ((ZombifiedPiglinEntity)entity).getRevengeTarget()
								  != null)
				.filter(entity
					-> !(entity instanceof EndermanEntity)
						   || ((EndermanEntity)entity).isScreaming())
				.forEach(entity
					-> res.add(new Avoidance(entity.getPosition(), mobCoeff,
			...
			for(int y = -radius; y <= radius; y++) {
				for(int z = -radius; z <= radius; z++) {
					if(x * x + y * y + z * z <= radius * radius) {
						long hash = BetterBlockPos.longHash(
