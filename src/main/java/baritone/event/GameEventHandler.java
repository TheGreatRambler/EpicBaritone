...
public final class GameEventHandler implements IEventBus, Helper {
	...
	public final void onTick(TickEvent event) {
		if(event.getType() == TickEvent.Type.IN) {
			try {
				baritone.bsi = new BlockStateInterface(
					baritone.getPlayerContext(), true);
			} catch(Exception ex) {
				ex.printStackTrace();
				baritone.bsi = null;
			}
	...
	public final void onChunkEvent(ChunkEvent event) {
		...
		boolean isPostPopulate
			= state == EventState.POST
			  && (type == ChunkEvent.Type.POPULATE_FULL
					 || type == ChunkEvent.Type.POPULATE_PARTIAL);

		World world = baritone.getPlayerContext().world();

		// Whenever the server sends us to another dimension, chunks are
		// unloaded technically after the new world has been loaded, so we
		// perform a check to make sure the chunk being unloaded is already
		// loaded.
		boolean isPreUnload = state == EventState.PRE
							  && type == ChunkEvent.Type.UNLOAD
							  && world.getChunkProvider().getChunk(
									 event.getX(), event.getZ(), null, false)
									 != null;
