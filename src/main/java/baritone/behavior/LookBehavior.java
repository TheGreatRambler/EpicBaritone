...
public final class LookBehavior extends Behavior implements ILookBehavior {
	...
	public void updateTarget(Rotation target, boolean force) {
		...
		if(!force) {
			...
			this.target = new Rotation(
				this.target.getYaw()
					+ (float)(rand
							  * Baritone.settings().randomLooking113.value),
	...
	public void onPlayerUpdate(PlayerUpdateEvent event) {
		...
		boolean silent
			= Baritone.settings().antiCheatCompatibility.value && !this.force;
		...
		case PRE: {
			if(this.force) {
				...
				ctx.player().rotationYaw
					+= (Math.random() - 0.5)
					   * Baritone.settings().randomLooking.value;
				ctx.player().rotationPitch
					+= (Math.random() - 0.5)
					   * Baritone.settings().randomLooking.value;
				if(desiredPitch == oldPitch
					&& !Baritone.settings().freeLook.value) {
					nudgeToLevel();
				}
				this.target = null;
			}
	...
	public void onPlayerRotationMove(RotationMoveEvent event) {
		if(this.target != null) {
			...
			// If we have antiCheatCompatibility on, we're going to use the
			// target value later in onPlayerUpdate() Also the type has to be
			// MOTION_UPDATE because that is called after JUMP
			if(!Baritone.settings().antiCheatCompatibility.value
				&& event.getType() == RotationMoveEvent.Type.MOTION_UPDATE
				&& !this.force) {
				this.target = null;
			}
		}
