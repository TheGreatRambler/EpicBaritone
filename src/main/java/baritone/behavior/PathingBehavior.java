...
public final class PathingBehavior
	extends Behavior implements IPathingBehavior, Helper {
		...
		synchronized(pathPlanLock) {
			synchronized(pathCalcLock) {
				if(inProgress != null) {
					...
					if((current == null
						   || !current.getPath().getDest().equals(
								  calcFrom)) // if current ends in inProgress's
											 // start, then we're ok
						&& !calcFrom.equals(ctx.playerFeet())
						&& !calcFrom.equals(
							   expectedSegmentStart) // if current starts in our
													 // playerFeet or pathStart,
													 // then we're ok
						&& (!currentBest.isPresent()
							   || (!currentBest.get().positions().contains(
									  ...
									  && !currentBest.get()
											  .positions()
											  .contains(
												  expectedSegmentStart))) // if
					) {
						// when it was *just* started, currentBest will be empty
						// so we need to also check calcFrom since that's always
						// present
						inProgress.cancel(); // cancellation doesn't dispatch
											 // any events
					}
				}
			}
			...
			if(current.failed() || current.finished()) {
				...
				if(next != null
					...
					&& !next.getPath().positions().contains(
						   expectedSegmentStart)) { // can contain either one
					// if the current path failed, we may not actually be on the
					// next one, so make sure
					logDebug(
						"Discarding next path as it does not contain current position");
					// for example if we had a nicely planned ahead path that
					// starts where current ends that's all fine and good but if
					// we fail in the middle of current we're nowhere close to
					// our planned ahead path so need to discard it sadly.
				...
				if(next != null) {
					...
					current = next;
					next    = null;
					current.onTick(); // don't waste a tick doing nothing, get
									  // started right away
					return;
				}
				// at this point, current just ended, but we aren't in the goal
				// and have no plan for the future
				synchronized(pathCalcLock) {
					if(inProgress != null) {
						queuePathEvent(
							PathEvent.PATH_FINISHED_NEXT_STILL_CALCULATING);
						return;
					}
			...
			synchronized(pathCalcLock) {
				...
				if(ticksRemainingInSegment(false).get()
					< Baritone.settings().planningTickLookahead.value) {
					// and this path has 7.5 seconds or less left
					// don't include the current movement so a very long last
					// movement (e.g. descend) doesn't trip it up if we actually
					// included current, it wouldn't start planning ahead until
					// the last movement was done, if the last movement took
					// more than 7.5 seconds on its own
					logDebug("Path almost over. Planning ahead...");
					queuePathEvent(PathEvent.NEXT_SEGMENT_CALC_STARTED);
					findPathInNewThread(
						current.getPath().getDest(), false, context);
				}
			}
		}
	}
	...
	public boolean cancelEverything() {
		...
		baritone.getPathingControlManager()
			.cancelEverything(); // regardless of if we can stop the current
								 // segment, we can still stop the processes
		return doIt;
	}
	...
	public void softCancelIfSafe() {
		synchronized(pathPlanLock) {
			getInProgress().ifPresent(
				AbstractNodeCostSearch::cancel); // only cancel ours
			if(!isSafeToCancel()) {
				return;
			}
			current = null;
	...
	@Override public void forceCancel() { // exposed on public api because :sob:
		cancelEverything();
		secretInternalSegmentCancel();
		synchronized(pathCalcLock) {
			inProgress = null;
			...
			if(ctx.player().isOnGround()) {
				...
				for(int dx = -1; dx <= 1; dx++) {
					for(int dz = -1; dz <= 1; dz++) {
						closest.add(new BetterBlockPos(
							feet.x + dx, feet.y, feet.z + dz));
					}
				}
				...
				for(int i = 0; i < 4; i++) {
					...
					double xDist
						= Math.abs((possibleSupport.x + 0.5D) - playerX);
					double zDist
						= Math.abs((possibleSupport.z + 0.5D) - playerZ);
					if(xDist > 0.8 && zDist > 0.8) {
						// can't possibly be sneaking off of this one, we're too
						// far away
						continue;
					}
					if(MovementHelper.canWalkOn(ctx, possibleSupport.down())
						...
						&& MovementHelper.canWalkThrough(
							   ctx, possibleSupport.up())) {
						// this is plausible
						// logDebug("Faking path start assuming player is
						// standing off the edge of a block");
				...
				if(MovementHelper.canWalkOn(ctx, feet.down().down())) {
					// logDebug("Faking path start assuming player is midair and
					// falling");
					return feet.down();
				}
			}
	 ...
	 * @param talkAboutIt
	 */
	private void findPathInNewThread(final BlockPos start,
		final boolean talkAboutIt, CalculationContext context) {
		...
		if(!Thread.holdsLock(pathCalcLock)) {
			throw new IllegalStateException(
				"Must be called with synchronization on pathCalcLock");
			// why do it this way? it's already indented so much that putting
			// the whole thing in a synchronized(pathCalcLock) was just too much
			// lol
		}
		if(inProgress != null) {
			throw new IllegalStateException(
		...
		if(!context.safeForThreadedUse) {
			throw new IllegalStateException(
				"Improper context thread safety level");
		}
		Goal goal = this.goal;
		if(goal == null) {
			logDebug("no goal"); // TODO should this be an exception too?
								 // definitely should be checked by caller
			return;
		}
		long primaryTimeout;
		long failureTimeout;
		...
		AbstractNodeCostSearch pathfinder = createPathfinder(
			start, goal, current == null ? null : current.getPath(), context);
		if(!Objects.equals(pathfinder.getGoal(),
			   goal)) { // will return the exact same object if simplification
						// didn't happen
			logDebug("Simplifying " + goal.getClass()
					 + " to GoalXZ due to distance");
		}
		inProgress = pathfinder;
		Baritone.getExecutor().execute(() -> {
			if(talkAboutIt) {
				logDebug("Starting to search for path from " + start + " to "
						 + goal);
			}
			...
			synchronized(pathPlanLock) {
				Optional<PathExecutor> executor = calcResult.getPath().map(
					p -> new PathExecutor(PathingBehavior.this, p));
				if(current == null) {
					...
					if(next == null) {
						if(executor.isPresent()) {
							if(executor.get().getPath().getSrc().equals(
								   current.getPath().getDest())) {
								queuePathEvent(
						...
						// throw new IllegalStateException("I have no idea what
						// to do with this path");
						// no point in throwing an exception here, and it gets
						// it stuck with inProgress being not null
						logDirect(
							"Warning: PathingBehaivor illegal state! Discarding invalid path!");
					}
				}
				if(talkAboutIt && current != null
					&& current.getPath() != null) {
					if(goal.isInGoal(current.getPath().getDest())) {
						logDebug("Finished finding a path from " + start
								 + " to " + goal + ". "
								 + current.getPath().getNumNodesConsidered()
								 + " nodes considered");
					} else {
						logDebug("Found path segment from " + start
								 + " towards " + goal + ". "
								 + current.getPath().getNumNodesConsidered()
								 + " nodes considered");
					}
	...
	private static AbstractNodeCostSearch createPathfinder(
		BlockPos start, Goal goal, IPath previous, CalculationContext context) {
		Goal transformed = goal;
		if(Baritone.settings().simplifyUnloadedYCoord.value
			&& goal instanceof IGoalRenderPos) {
		...
		Favoring favoring = new Favoring(
			context.getBaritone().getPlayerContext(), previous, context);
		return new AStarPathFinder(start.getX(), start.getY(), start.getZ(),
			transformed, favoring, context);
	}
