...
public class WaypointCollection implements IWaypointCollection {
	...
	private synchronized void load(Waypoint.Tag tag) {
		...
		Path fileName
			= this.directory.resolve(tag.name().toLowerCase() + ".mp4");
		...
		try(FileInputStream fileIn    = new FileInputStream(fileName.toFile());
			...
			long length = in.readLong(); // Yes I want 9,223,372,036,854,775,807
										 // waypoints, do you not?
			while(length-- > 0) {
				...
				int y                  = in.readInt();
				int z                  = in.readInt();
				this.waypoints.get(tag).add(new Waypoint(
	...
	public IWaypoint getMostRecentByTag(IWaypoint.Tag tag) {
		// Find a waypoint of the given tag which has the greatest timestamp
		// value, indicating the most recent
		return this.waypoints.get(tag)
			.stream()
