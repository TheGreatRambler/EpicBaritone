...
public final class CachedRegion implements ICachedRegion {
	...
	/**
	 * Magic value to detect invalid cache files, or incompatible cache files
	 * saved in an old version of Baritone
	 */
		...
		for(int chunkX = 0; chunkX < 32; chunkX++) {
			for(int chunkZ = 0; chunkZ < 32; chunkZ++) {
				...
				ArrayList<BlockPos> locs
					= chunks[chunkX][chunkZ].getAbsoluteBlocks(block);
				if(locs != null) {
					res.addAll(locs);
				}
			}
	...
	public final synchronized void updateCachedChunk(
		int chunkX, int chunkZ, CachedChunk chunk) {
	...
	public synchronized final void save(String directory) {
		...
		try {
			...
			System.out.println(
				"Saving region " + x + "," + z + " to disk " + path);
			Path regionFile = getRegionFile(path, this.x, this.z);
			if(!Files.exists(regionFile)) {
				Files.createFile(regionFile);
			}
			try(FileOutputStream fileOut
				= new FileOutputStream(regionFile.toFile());
				...
				for(int x = 0; x < 32; x++) {
					for(int z = 0; z < 32; z++) {
						...
						if(chunk == null) {
							...
							// Messy, but fills the empty 0s that should be
							// trailing to fill up the space.
							out.write(new byte[CachedChunk.SIZE_IN_BYTES
											   - chunkBytes.length]);
						}
					}
				}
				for(int x = 0; x < 32; x++) {
					for(int z = 0; z < 32; z++) {
						if(chunks[x][z] != null) {
							for(int i = 0; i < 256; i++) {
								out.writeUTF(BlockUtils.blockToString(
				...
				for(int x = 0; x < 32; x++) {
					for(int z = 0; z < 32; z++) {
						if(chunks[x][z] != null) {
							Map<String, List<BlockPos>> locs
								= chunks[x][z].getRelativeBlocks();
	...
	public synchronized void load(String directory) {
		try {
			...
			System.out.println(
				"Loading region " + x + "," + z + " from disk " + path);
			long start = System.nanoTime() / 1000000L;

			try(FileInputStream fileIn
				= new FileInputStream(regionFile.toFile());
				...
				if(magic != CACHED_REGION_MAGIC) {
					...
					// by switching on the magic value, and either loading it
					// normally, or loading through a converter.
					...
					for(int z = 0; z < 32; z++) {
						if(present[x][z]) {
							for(int i = 0; i < 256; i++) {
								overview[x][z][i]
									= BlockUtils
										  .stringToBlockRequired(in.readUTF())
										  .getDefaultState();
							}
						}
					}
				...
				for(int x = 0; x < 32; x++) {
					for(int z = 0; z < 32; z++) {
						if(present[x][z]) {
							...
							// ^ haha jokes on leijurv, java doesn't have
							// unsigned types so that isn't correct
							//   also why would you have more than 32767 special
							//   blocks in a chunk
							// haha double jokes on you now it works for 65535
							// not just 32767
							...
							for(int i = 0; i < numSpecialBlockTypes; i++) {
								...
								if(numLocations == 0) {
									// an entire chunk full of air can happen in
									// the end
									numLocations = 65536;
								}
				...
				// only if the entire file was uncorrupted do we actually set
				// the chunks
				for(int x = 0; x < 32; x++) {
					for(int z = 0; z < 32; z++) {
						if(present[x][z]) {
							...
							this.chunks[x][z] = new CachedChunk(chunkX, chunkZ,
			...
			System.out.println(
				"Loaded region successfully in " + (end - start) + "ms");
		} catch(Exception ex) { // corrupted files can cause
								// NullPointerExceptions as well as IOExceptions
			ex.printStackTrace();
		}
	}
		...
		for(int x = 0; x < 32; x++) {
			for(int z = 0; z < 32; z++) {
				if(this.chunks[x][z] != null
					...
					System.out.println(
						"Removing chunk " + (x + 32 * this.x) + ","
						+ (z + 32 * this.z) + " because it was cached "
						+ (now - this.chunks[x][z].cacheTimestamp) / 1000L
						+ " seconds ago, and max age is " + expiry);
	...
	public synchronized final CachedChunk mostRecentlyModified() {
		...
		for(int x = 0; x < 32; x++) {
			for(int z = 0; z < 32; z++) {
				...
				if(recent == null
					|| this.chunks[x][z].cacheTimestamp
						   > recent.cacheTimestamp) {
	...
	private static Path getRegionFile(Path cacheDir, int regionX, int regionZ) {
		return Paths.get(
			cacheDir.toString(), "r." + regionX + "." + regionZ + ".bcr");
	}
}
