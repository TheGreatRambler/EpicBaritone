...
public enum WorldScanner implements IWorldScanner {
	...
	@Override
	public List<BlockPos> scanChunkRadius(IPlayerContext ctx,
		...
		ClientChunkProvider chunkProvider
			= (ClientChunkProvider)ctx.world().getChunkProvider();
		...
		int[] coordinateIterationOrder
			= IntStream.range(0, 16)
				  ...
				  .sorted(Comparator.comparingInt(
					  y -> Math.abs(y - playerYBlockStateContainerIndex)))
				  .mapToInt(x -> x)
				  .toArray();
		...
		while(true) {
			...
			for(int xoff = -searchRadiusSq; xoff <= searchRadiusSq; xoff++) {
				for(int zoff = -searchRadiusSq; zoff <= searchRadiusSq;
					zoff++) {
					...
					Chunk chunk
						= chunkProvider.getChunk(chunkX, chunkZ, null, false);
					if(chunk == null) {
						continue;
					}
					allUnloaded = false;
					if(scanChunkInto(chunkX << 4, chunkZ << 4, chunk, filter,
						   res, max, yLevelThreshold, playerY,
	...
	@Override
	public List<BlockPos> scanChunk(IPlayerContext ctx,
		BlockOptionalMetaLookup filter, ChunkPos pos, int max,
		int yLevelThreshold) {
		if(filter.blocks().isEmpty()) {
		...
		ArrayList<BlockPos> res = new ArrayList<>();
		scanChunkInto(pos.x << 4, pos.z << 4, chunk, filter, res, max,
			yLevelThreshold, playerY, DEFAULT_COORDINATE_ITERATION_ORDER);
		return res;
	}
	...
	private boolean scanChunkInto(int chunkX, int chunkZ, Chunk chunk,
		BlockOptionalMetaLookup filter, Collection<BlockPos> result, int max,
		int yLevelThreshold, int playerY, int[] coordinateIterationOrder) {
		...
		for(int yIndex = 0; yIndex < 16; yIndex++) {
			...
			// creating this array and then using getAtPalette is faster than
			// even getFast(int index)
					...
					if(result.size() >= max) {
						if(Math.abs(y - playerY) < yLevelThreshold) {
							...
							if(foundWithinY) {
								...
								// need to consider outside Y
								// TODO continue iteration to one more sorted Y
								// coordinate block
								return true;
							}
						}
