...
public class PathExecutor implements IPathExecutor, Helper {
	...
	/**
	 * Default value is equal to 10 seconds. It's find to decrease it, but it
	 * must be at least 5.5s (110 ticks). For more information, see issue #102.
	 *
	 ...
	 * Tick this executor
	 *
	 * @return True if a movement just finished (and the player is therefore in
	 * a "stable" state, like, not sneaking out over lava), false otherwise
	 */
	public boolean onTick() {
		...
		if(!movement.getValidPositions().contains(whereAmI)) {
			for(int i = 0; i < pathPosition && i < path.length();
				i++) { // this happens for example when you lag out and get
					   // teleported back a couple blocks
			...
			for(int i = pathPosition + 3; i < path.length() - 1;
				i++) { // dont check pathPosition+1. the movement tells us when
					   // it's done (e.g. sneak placing)
				// also don't check pathPosition+2 because reasons
				if(((Movement)path.movements().get(i))
						.getValidPositions()
						.contains(whereAmI)) {
					if(i - pathPosition > 2) {
						logDebug("Skipping forward " + (i - pathPosition)
								 + " steps, to " + i);
					}
		...
		if(possiblyOffPath(status, MAX_DIST_FROM_PATH)) {
			ticksAway++;
			System.out.println("FAR AWAY FROM PATH FOR " + ticksAway
							   + " TICKS. Current distance: " + status.getA()
							   + ". Threshold: " + MAX_DIST_FROM_PATH);
			if(ticksAway > MAX_TICKS_AWAY) {
				logDebug(
					"Too far away from path for too long, cancelling path");
				cancel();
				return false;
			}
		} else {
		...
		if(possiblyOffPath(
			   status, MAX_MAX_DIST_FROM_PATH)) { // ok, stop right away, we're
												  // way too far.
		...
		if (end - start > 0) {
			System.out.println("Recalculating break and place took " + (end -
		start) + "ms");
		}*/
		if(pathPosition < path.movements().size() - 1) {
			IMovement next = path.movements().get(pathPosition + 1);
			if(!behavior.baritone.bsi.worldContainsLoadedChunk(
				   next.getDest().x, next.getDest().z)) {
				logDebug(
					"Pausing since destination is at edge of loaded chunks");
				clearKeys();
				return true;
			}
		}
		...
		if(costEstimateIndex == null || costEstimateIndex != pathPosition) {
			...
			// do this only once, when the movement starts, and deliberately get
			// the cost as cached when this path was calculated, not the cost as
			// it is right now
			currentMovementOriginalCostEstimate = movement.getCost();
			for(int i = 1;
				i < Baritone.settings().costVerificationLookahead.value
		...
		if(!movement.calculatedWhileLoaded()
			...
			// that means that this isn't a cache error, it's just part of the
			// path interfering with a later part
			logDebug("Original cost " + currentMovementOriginalCostEstimate
					 + " current cost " + currentCost + ". Cancelling.");
			cancel();
			return true;
		}
		if(shouldPause()) {
		...
		if(movementStatus == SUCCESS) {
			...
			if(!sprintNextTick) {
				ctx.player().setSprinting(
					false); // letting go of control doesn't make you stop
							// sprinting actually
			}
			ticksOnCurrent++;
			if(ticksOnCurrent
				...
				// only cancel if the total time has exceeded the initial
				// estimate as you break the blocks required, the remaining cost
				// goes down, to the point where ticksOnCurrent is greater than
				// recalculateCost + 100 this is why we cache cost at the
				// beginning, and don't recalculate for this comparison every
				// tick
				logDebug("This movement has taken too long (" + ticksOnCurrent
						 + " ticks, expected "
						 + currentMovementOriginalCostEstimate
						 + "). Cancelling.");
				cancel();
		...
		return canCancel; // movement is in progress, but if it reports
						  // cancellable, PathingBehavior is good to cut onto
						  // the next path
	}
	...
	private boolean shouldPause() {
		...
		if(positions.size() < 3) {
			return false; // not long enough yet to justify pausing, its far
						  // from certain we'll actually take this route
		}
		// the first block of the next path will always overlap
		// no need to pause our very last movement when it would have otherwise
		// cleanly exited with MovementStatus SUCCESS
		positions = positions.subList(1, positions.size());
		return positions.contains(ctx.playerFeet());
	}

	private boolean possiblyOffPath(
		Tuple<Double, BlockPos> status, double leniency) {
		double distanceFromPath = status.getA();
		if(distanceFromPath > leniency) {
			// when we're midair in the middle of a fall, we're very far from
			// both the beginning and the end, but we aren't actually off path
			if(path.movements().get(pathPosition) instanceof MovementFall) {
				BlockPos fallDest = path.positions().get(
					pathPosition
					+ 1); // .get(pathPosition) is the block we fell off of
				return VecUtils.entityFlatDistanceToCenter(
						   ctx.player(), fallDest)
	...
	public boolean snipsnapifpossible() {
		if(!ctx.player().isOnGround()
			...
			if(ctx.player().getMotion().y < -0.1) {
				...
				// we could be falling through water, which could be unsafe to
				// splice
	...
	private boolean shouldSprintNextTick() {
		boolean requested
			= behavior.baritone.getInputOverrideHandler().isInputForcedDown(
				Input.SPRINT);

		// we'll take it from here, no need for minecraft to see we're holding
		// down control and sprint for us
		behavior.baritone.getInputOverrideHandler().setInputForceState(
			Input.SPRINT, false);

		// first and foremost, if allowSprint is off, or if we don't have enough
		// hunger, don't try and sprint
		...
		if(current instanceof MovementTraverse
			...
			if(next instanceof MovementAscend
				...
				if(skipNow(ctx, current)) {
					...
					onChangeInPathPosition();
					onTick();
					behavior.baritone.getInputOverrideHandler()
		...
		// however, descend and ascend don't request sprinting, because they
		// don't know the context of what movement comes after it
		if(current instanceof MovementDescend) {
			...
			if(pathPosition < path.length() - 2) {
				...
				if(next instanceof MovementAscend
					...
					onChangeInPathPosition();
					onTick();
					// okay to skip clearKeys and / or onChangeInPathPosition
					// here since this isn't possible to repeat, since it's
					// asymmetric
		...
		if(current instanceof MovementFall) {
			Tuple<Vector3d, BlockPos> data
				= overrideFall((MovementFall)current);
			if(data != null) {
	...
	private Tuple<Vector3d, BlockPos> overrideFall(MovementFall movement) {
		...
		int i;
	outer:
		for(i = pathPosition + 1; i < path.length() - 1 && i < pathPosition + 3;
			i++) {
			...
			for(int y = next.getDest().y; y <= movement.getSrc().y + 1; y++) {
				BlockPos chk
					= new BlockPos(next.getDest().x, y, next.getDest().z);
		...
		return new Tuple<>(
			new Vector3d(flatDir.getX() * len + movement.getDest().x + 0.5,
				movement.getDest().y,
				flatDir.getZ() * len + movement.getDest().z + 0.5),
			movement.getDest().add(flatDir.getX() * (i - pathPosition), 0,
	...
	private static boolean skipNow(IPlayerContext ctx, IMovement current) {
		...
		// we are centered
		BlockPos headBonk
			= current.getSrc().subtract(current.getDirection()).up(2);
	...
	private static boolean sprintableAscend(IPlayerContext ctx,
		MovementTraverse current, MovementAscend next, IMovement nextnext) {
		...
		if(MovementHelper.avoidWalkingInto(
			   ctx.world().getBlockState(current.getSrc().up(3)))) {
			return false;
		}
		return !MovementHelper.avoidWalkingInto(ctx.world().getBlockState(
			next.getDest().up(2))); // codacy smh my head
	}
	...
	private static boolean canSprintFromDescendInto(
		...
		if(!MovementHelper.canWalkOn(
			   ctx, current.getDest().add(current.getDirection()))) {
			return false;
		}
		if(next instanceof MovementTraverse
			&& next.getDirection().down().equals(current.getDirection())) {
			return true;
		}
	...
	private PathExecutor cutIfTooLong() {
		if(pathPosition > Baritone.settings().maxPathHistoryLength.value) {
			...
			CutoffPath newPath
				= new CutoffPath(path, cutoffAmt, path.length() - 1);
			if(!newPath.getDest().equals(path.getDest())) {
				throw new IllegalStateException();
			}
			logDebug("Discarding earliest segment movements, length cut from "
					 + path.length() + " to " + newPath.length());
