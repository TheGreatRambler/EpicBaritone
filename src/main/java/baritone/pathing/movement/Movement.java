...
public abstract class Movement implements IMovement, MovementHelper {
	...
	private MovementState currentState
		= new MovementState().setStatus(MovementStatus.PREPPING);
	...
	/**
	 * The position where we need to place a block before this movement can
	 * ensue
	 */
	...
	protected Movement(IBaritone baritone, BetterBlockPos src,
		BetterBlockPos dest, BetterBlockPos[] toBreak, BetterBlockPos toPlace) {
	...
	protected Movement(IBaritone baritone, BetterBlockPos src,
		BetterBlockPos dest, BetterBlockPos[] toBreak) {
	...
	public MovementStatus update() {
		...
		// If the movement target has to force the new rotations, or we aren't
		// using silent move, then force the rotations
		currentState.getTarget().getRotation().ifPresent(rotation
			-> baritone.getLookBehavior().updateTarget(
				rotation, currentState.getTarget().hasToForceRotations()));
	...
	protected boolean prepared(MovementState state) {
		...
		for(BetterBlockPos blockPos : positionsToBreak) {
			if(!ctx.world()
					.getEntitiesWithinAABB(FallingBlockEntity.class,
			...
			if(!MovementHelper.canWalkThrough(
				   ctx, blockPos)) { // can't break air, so don't try
				somethingInTheWay = true;
				MovementHelper.switchToBestToolFor(
					ctx, BlockStateInterface.get(ctx, blockPos));
				Optional<Rotation> reachable
					= RotationUtils.reachable(ctx.player(), blockPos,
				...
				state.setTarget(new MovementState.MovementTarget(
					RotationUtils.calcRotationFromVec3d(ctx.playerHead(),
						...
						ctx.playerRotations()),
					true));
				// don't check selectedblock on this one, this is a fallback
				// when we can't see any face directly, it's intended to be
				// breaking the "incorrect" block
				state.setInput(Input.CLICK_LEFT, true);
				return false;
			}
		}
		if(somethingInTheWay) {
			// There's a block or blocks that we can't walk through, but we have
			// no target rotation to reach any So don't return true, actually
	...
	public void checkLoadedChunk(CalculationContext context) {
		calculatedWhileLoaded
			= context.bsi.worldContainsLoadedChunk(dest.x, dest.z);
	}
	...
	public List<BlockPos> toBreak(BlockStateInterface bsi) {
		...
		for(BetterBlockPos positionToBreak : positionsToBreak) {
			if(!MovementHelper.canWalkThrough(bsi, positionToBreak.x,
				   positionToBreak.y, positionToBreak.z)) {
	...
	public List<BlockPos> toWalkInto(
		BlockStateInterface bsi) { // overridden by movementdiagonal
