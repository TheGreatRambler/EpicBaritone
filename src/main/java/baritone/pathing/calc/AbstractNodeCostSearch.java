...
public abstract class AbstractNodeCostSearch implements IPathFinder, Helper {
	...
	/**
	 * This is really complicated and hard to explain. I wrote a comment in the
	 * old version of MineBot but it was so long it was easier as a Google Doc
	 * (because I could insert charts).
	 *
	 * @see <a
	 * href="https://docs.google.com/document/d/1WVHHXKXFdCR1Oz__KtK8sFqyvSwJN_H4lftkHFgmzlc/edit">here</a>
	 */
	protected static final double[] COEFFICIENTS = { 1.5, 2, 2.5, 3, 4, 5, 10 };

	/**
	 * If a path goes less than 5 blocks and doesn't make it to its goal, it's
	 * not worth considering.
	 */
	protected static final double MIN_DIST_PATH = 5;

	/**
	 * there are floating point errors caused by random combinations of traverse
	 * and diagonal over a flat area that means that sometimes there's a cost
	 * improvement of like 10 ^ -16 it's not worth the time to update the costs,
	 * decrease-key the heap, potentially repropagate, etc <p> who cares about a
	 * hundredth of a tick? that's half a millisecond for crying out loud!
	 */
	...
	@Override
	public synchronized PathCalculationResult calculate(
		...
		try {
			IPath path = calculate0(primaryTimeout, failureTimeout)
							 .map(IPath::postProcess)
							 .orElse(null);
			if(cancelRequested) {
			...
			if(path == null) {
				return new PathCalculationResult(
					PathCalculationResult.Type.FAILURE);
			}
			...
			if(path.length() < previousLength) {
				Helper.HELPER.logDebug(
					"Cutting off path at edge of loaded chunks");
				Helper.HELPER.logDebug(
					"Length decreased by " + (previousLength - path.length()));
			} else {
			...
			if(path.length() < previousLength) {
				Helper.HELPER.logDebug(
					"Static cutoff " + previousLength + " to " + path.length());
			}
			if(goal.isInGoal(path.getDest())) {
				return new PathCalculationResult(
					PathCalculationResult.Type.SUCCESS_TO_GOAL, path);
			} else {
				return new PathCalculationResult(
					PathCalculationResult.Type.SUCCESS_SEGMENT, path);
			}
		} catch(Exception e) {
			...
			// this is run regardless of what exception may or may not be raised
			// by calculate0
			isFinished = true;
		}
	}
	...
	/**
	 * Attempts to search the block position hashCode long to {@link PathNode}
	 * map for the node mapped to the specified pos. If no node is found, a new
	 * node is created.
	 *
	 * @param x        The x position of the node
	 * @param y        The y position of the node
	 * @param z        The z position of the node
	 * @param hashCode The hash code of the node, provided by {@link
	 * BetterBlockPos#longHash(int, int, int)}
	...
	public Optional<IPath> pathToMostRecentNodeConsidered() {
		return Optional.ofNullable(mostRecentConsidered)
			.map(node -> new Path(startNode, node, 0, goal, context));
	}
	...
	protected Optional<IPath> bestSoFar(boolean logInfo, int numNodes) {
		...
		for(int i = 0; i < COEFFICIENTS.length; i++) {
			...
			if(dist > MIN_DIST_PATH
						  * MIN_DIST_PATH) { // square the comparison since
											 // distFromStartSq is squared
				if(logInfo) {
					if(COEFFICIENTS[i] >= 3) {
						System.out.println(
						...
						System.out.println(
							"But I'm going to do it anyway, because yolo");
					}
					System.out.println(
				...
				return Optional.of(
					new Path(startNode, bestSoFar[i], numNodes, goal, context));
			}
		}
		...
		if(logInfo) {
			logDebug("Even with a cost coefficient of "
					 + COEFFICIENTS[COEFFICIENTS.length - 1]
					 + ", I couldn't get more than " + Math.sqrt(bestDist)
					 + " blocks");
