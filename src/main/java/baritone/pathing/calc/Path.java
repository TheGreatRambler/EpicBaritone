...
class Path extends PathBase {
	...
	Path(PathNode start, PathNode end, int numNodes, Goal goal,
		...
		while(current != null) {
			...
			tempPath.addFirst(
				new BetterBlockPos(current.x, current.y, current.z));
			current = current.previous;
		}
		// Can't directly convert from the PathNode pseudo linked list to an
		// array because we don't know how long it is inserting into a
		// LinkedList<E> keeps track of length, then when we addall (which calls
		// .toArray) it's able to performantly do that conversion since it knows
		// the length.
	...
	public IPath postProcess() {
		...
		if(failed) { // at least one movement became impossible during
					 // calculation
