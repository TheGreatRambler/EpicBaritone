...
public class ExploreFilterCommand extends Command {
	...
	public void execute(String label, IArgConsumer args)
		...
		File file      = args.getDatatypePost(RelativeFile.INSTANCE,
            mc.gameDir.getAbsoluteFile().getParentFile());
		...
		try {
			baritone.getExploreProcess().applyJsonFilter(
				file.toPath().toAbsolutePath(), invert);
		...
		logDirect(String.format(
			"Explore filter applied. Inverted: %s", Boolean.toString(invert)));
	}
