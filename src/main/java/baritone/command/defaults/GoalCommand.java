...
public class GoalCommand extends Command {
	...
	public void execute(String label, IArgConsumer args)
		...
		if(args.hasAny()
			&& Arrays.asList("reset", "clear", "none")
				   .contains(args.peekString())) {
			args.requireMax(1);
	...
	public Stream<String> tabComplete(String label, IArgConsumer args)
		...
		if(args.hasExactlyOne()) {
			...
			if(args.hasAtMost(3)) {
				while(args.has(2)) {
					if(args.peekDatatypeOrNull(RelativeCoordinate.INSTANCE)
						== null) {
						break;
					}
					args.get();
	...
	public List<String> getLongDesc() {
		return Arrays.asList(
			...
			"", "Usage:", "> goal - Set the goal to your current position",
