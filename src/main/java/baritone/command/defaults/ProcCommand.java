...
public class ProcCommand extends Command {
	...
	public void execute(String label, IArgConsumer args)
		...
		args.requireMax(0);
		IPathingControlManager pathingControlManager
			= baritone.getPathingControlManager();
		IBaritoneProcess process
			= pathingControlManager.mostRecentInControl().orElse(null);
		if(process == null) {
		...
		logDirect(String.format("Class: %s\n"
									...
									+ "Display name: %s\n"
									+ "Last command: %s",
			process.getClass().getTypeName(), process.priority(),
			process.isTemporary(), process.displayName(),
			pathingControlManager.mostRecentCommand()
	...
	public List<String> getLongDesc() {
		return Arrays.asList(
			...
			"", "Usage:", "> proc - View process information, if present");
	}
}
