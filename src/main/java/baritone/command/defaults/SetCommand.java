...
public class SetCommand extends Command {
	...
	public void execute(String label, IArgConsumer args)
		...
		String arg
			= args.hasAny() ? args.getString().toLowerCase(Locale.US) : "list";
		...
		boolean viewModified
			= Arrays.asList("m", "mod", "modified").contains(arg);
		...
		if(paginate) {
			...
			List<? extends Settings.Setting> toPaginate
				= (viewModified
						...
						: Baritone.settings().allSettings)
					  .stream()
					  .filter(s -> !s.getName().equals("logger"))
					  .filter(s
						  -> s.getName().toLowerCase(Locale.US).contains(
							  search.toLowerCase(Locale.US)))
					  .sorted((s1, s2)
								  -> String.CASE_INSENSITIVE_ORDER.compare(
									  s1.getName(), s2.getName()))
					  .collect(Collectors.toList());
			Paginator.paginate(args, new Paginator<>(toPaginate),
				()
					-> logDirect(
						!search.isEmpty()
							? String.format(
				...
				-> {
					...
					TextComponent component
						= new StringTextComponent(setting.getName());
					component.setStyle(component.getStyle().setFormatting(
						TextFormatting.GRAY));
		...
		if(resetting) {
			if(!args.hasAny()) {
				logDirect(
					"Please specify 'all' as an argument to reset to confirm you'd really like to do this");
				logDirect(
					"ALL settings will be reset. Use the 'set modified' or 'modified' commands to see what will be reset.");
				logDirect(
		...
		if(!doingSomething && !args.hasAny()) {
			...
			if(!toggling) {
				logDirect(String.format("Successfully %s %s to %s",
					resetting ? "reset" : "set", setting.getName(),
					settingValueToString(setting)));
			}
			TextComponent oldValueComponent = new StringTextComponent(
				String.format("Old value: %s", oldValue));
				...
				oldValueComponent.getStyle()
					...
					.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
						FORCE_COMMAND_PREFIX
							+ String.format(
								  "set %s %s", setting.getName(), oldValue))));
			...
			if((setting.getName().equals("chatControl")
				...
				logDirect(
					...
					TextFormatting.RED);
			} else if(setting.getName().equals("prefixControl")
					  && !(Boolean)setting.value) {
				logDirect(
	...
	public Stream<String> tabComplete(String label, IArgConsumer args)
		...
		if(args.hasAny()) {
			...
			if(args.hasExactlyOne()
				...
				Settings.Setting setting = Baritone.settings().byLowerName.get(
					arg.toLowerCase(Locale.US));
				if(setting != null) {
