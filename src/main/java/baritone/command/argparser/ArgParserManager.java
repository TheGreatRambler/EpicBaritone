...
public enum ArgParserManager implements IArgParserManager {
	...
	@Override
	public <T, S> IArgParser.Stated<T, S> getParserStated(
		...
		return this.registry.descendingStream()
			...
			.filter(parser -> parser.getTarget().isAssignableFrom(type))
			.filter(
				parser -> parser.getStateType().isAssignableFrom(stateKlass))
			.map(IArgParser.Stated.class ::cast)
			.findFirst()
			.orElse(null);
	}
	...
	@Override
	public <T, S> T parseStated(Class<T> type, Class<S> stateKlass,
