...
public class CommandManager implements ICommandManager {
	...
	private ExecutionWrapper from(
		...
		return command == null ? null
							   : new ExecutionWrapper(command, label, args);
	}
	...
	private static Tuple<String, List<ICommandArgument>> expand(
		...
		List<ICommandArgument> args = CommandArguments.from(
			string.substring(label.length()), preserveEmptyLast);
	...
	private static final class ExecutionWrapper {
		...
		private ExecutionWrapper(
			ICommand command, String label, ArgConsumer args) {
