...
public class ArgConsumer implements IArgConsumer {
	...
	/**
	 * The parent {@link ICommandManager} for this {@link IArgConsumer}}. Used
	 * by {@link #context}.
	 */
	private final ICommandManager manager;

	/**
	 * The {@link IDatatypeContext} instance for this {@link IArgConsumer}},
	 * passed to datatypes when an operation is performed upon them.
	 *
	 * @see IDatatype
	...
	private ArgConsumer(ICommandManager manager, Deque<ICommandArgument> args,
	...
	@Override
	public <T, O, D extends IDatatypePost<T, O>> T peekDatatypePost(
	...
	@Override
	public <T, O, D extends IDatatypePost<T, O>> T peekDatatypePostOrNull(
		D datatype, O original) {
	...
	@Override
	public <T, D extends IDatatypeFor<T>> T peekDatatypeForOrDefault(
		Class<D> datatype, T def) {
		return copy().peekDatatypeForOrDefault(datatype, def);
	}

	@Override
	public <T, D extends IDatatypeFor<T>> T peekDatatypeForOrNull(
		Class<D> datatype) {
	...
	@Override
	public <T, O, D extends IDatatypePost<T, O>> T getDatatypePost(
		D datatype, O original)
		throws CommandInvalidTypeException, CommandNotEnoughArgumentsException {
		try {
			...
			throw new CommandInvalidTypeException(
				hasAny() ? peek() : consumed(),
				datatype.getClass().getSimpleName(), e);
		}
	}
	...
	public <T, O, D extends IDatatypePost<T, O>> T getDatatypePostOrDefault(
		...
		final List<ICommandArgument> consumedSnapshot
			= new ArrayList<>(this.consumed);
		try {
	...
	@Override
	public <T, O, D extends IDatatypePost<T, O>> T getDatatypePostOrNull(
		D datatype, O original) {
	...
	@Override
	public <T, D extends IDatatypeFor<T>> T getDatatypeForOrDefault(
		D datatype, T def) {
		final List<ICommandArgument> argsSnapshot = new ArrayList<>(this.args);
		final List<ICommandArgument> consumedSnapshot
			= new ArrayList<>(this.consumed);
	...
	@Override
	public <T extends IDatatype> Stream<String> tabCompleteDatatype(
		T datatype) {
		try {
	...
	public ICommandArgument consumed() {
		return consumed.size() > 0 ? consumed.getLast()
