...
public final class BackfillProcess extends BaritoneProcessHelper {
	...
	public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
		...
		for(BlockPos toPlace : toFillIn()) {
			...
			switch(MovementHelper.attemptToPlaceABlock(
				fake, baritone, toPlace, false, false)) {
			case NO_OPTION:
				continue;
			case READY_TO_PLACE:
			...
			case ATTEMPTING:
				...
				baritone.getLookBehavior().updateTarget(
					fake.getTarget().getRotation().get(), true);
				return new PathingCommand(
					null, PathingCommandType.REQUEST_PAUSE);
			default:
		...
		return new PathingCommand(
			null, PathingCommandType.DEFER); // cede to other process
	}
	...
	private void amIBreakingABlockHMMMMMMM() {
		...
		blocksToReplace.put(ctx.getSelectedBlock().get(),
	...
	public List<BlockPos> toFillIn() {
		return blocksToReplace.keySet()
			.stream()
			.filter(
				pos -> ctx.world().getBlockState(pos).getBlock() == Blocks.AIR)
			.filter(pos
				-> baritone.getBuilderProcess().placementPlausible(
					pos, Blocks.DIRT.getDefaultState()))
			.filter(pos -> !partOfCurrentMovement(pos))
			.sorted(Comparator
						.<BlockPos>comparingDouble(ctx.playerFeet()::distanceSq)
						.reversed())
	...
	private boolean partOfCurrentMovement(BlockPos pos) {
		...
		Movement movement
			= (Movement)exec.getPath().movements().get(exec.getPosition());
