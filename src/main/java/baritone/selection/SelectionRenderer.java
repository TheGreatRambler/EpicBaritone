...
public class SelectionRenderer implements IRenderer, AbstractGameEventListener {
	...
	public static void renderSelections(
		MatrixStack stack, ISelection[] selections) {
		...
		for(ISelection selection : selections) {
			IRenderer.drawAABB(
				stack, selection.aabb(), SELECTION_BOX_EXPANSION);
		}
		...
		if(settings.renderSelectionCorners.value) {
			...
			for(ISelection selection : selections) {
				IRenderer.drawAABB(stack, new AxisAlignedBB(selection.pos1(),
											  selection.pos1().add(1, 1, 1)));
			}

			IRenderer.glColor(settings.colorSelectionPos2.value, opacity);

			for(ISelection selection : selections) {
				IRenderer.drawAABB(stack, new AxisAlignedBB(selection.pos2(),
											  selection.pos2().add(1, 1, 1)));
			}
		}
