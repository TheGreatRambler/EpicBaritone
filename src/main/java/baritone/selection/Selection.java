...
public class Selection implements ISelection {
	...
	public Selection(BetterBlockPos pos1, BetterBlockPos pos2) {
		...
		this.min = new BetterBlockPos(Math.min(pos1.x, pos2.x),
			Math.min(pos1.y, pos2.y), Math.min(pos1.z, pos2.z));

		this.max = new BetterBlockPos(Math.max(pos1.x, pos2.x),
	 ...
	 * <p>
	 * Let's say you specify Direction.UP, this functions returns if pos2 is the
	 * highest BlockPos. If you specify Direction.DOWN, it returns if pos2 is
	 * the lowest BlockPos.
	 *
	 * @param facing The direction to check.
	 * @return {@code true} if pos2 is further in that direction than pos1,
	 * {@code false} if it isn't, and something else if they're both at the same
	 * position on that axis (it really doesn't matter)
	 */
	...
	public ISelection shift(Direction direction, int blocks) {
		return new Selection(
			pos1.offset(direction, blocks), pos2.offset(direction, blocks));
	}
}
