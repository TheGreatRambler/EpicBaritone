...
public class OpenSetsTest {
	...
	private static void removeAndTest(
		...
		for(int i = 0; i < test.length; i++) {
			...
			System.out.println(test[i].getClass() + " "
							   + (System.nanoTime() / 1000000L - before));
		}
		for(int j = 0; j < amount; j++) {
	...
	public void testSize() {
		...
		// Include LinkedListOpenSet even though it's not performant because I
		// absolutely trust that it behaves properly I'm really testing the heap
		// implementations against it as the ground truth
		IOpenSet[] test = new IOpenSet[] { new BinaryHeapOpenSet(),
			new LinkedListOpenSet() };
		...
		for(IOpenSet set : test) {
			...
			System.out.println(
				set.getClass() + " " + (System.nanoTime() / 1000000L - before));
			// all three take either 0 or 1ms to insert up to 10,000 nodes
			// linkedlist takes 0ms most often (because there's no array
			// resizing or allocation there, just pointer shuffling)
		}
		...
		// remove a quarter of the nodes and verify that they are indeed the
		// size/4 lowest ones
		...
		for(int i = 0; cnt < size / 2 && i < size; i++) {
			if(lowestQuarter.contains(
				   toInsert[i])) { // these were already removed and can't be
								   // updated to test
				continue;
			}
			toInsert[i].combinedCost *= Math.random();
			// multiplying it by a random number between 0 and 1 is guaranteed
			// to decrease it
			for(IOpenSet set : test) {
				// it's difficult to benchmark these individually because if you
				// modify all at once then update then it breaks the internal
				// consistency of the heaps. you have to call update every time
				// you modify a node.
